...
pars = argparse.ArgumentParser(formatter_class=argparse.RawDescriptionHelpFormatter, description=''' Filter by YY 20230314''')
...
pars.add_argument('-r', metavar='<str>', type=str, help='''input ref file or dir.''', required=False, default = '')
pars.add_argument('-q1', metavar='<str>', type=str, help='''input fq/gz -1 files.''', required=False, nargs="+")
pars.add_argument('-q2', metavar='<str>', type=str, help='''input fq/gz -2 files.''', required=False, nargs="+")
pars.add_argument('-kf', metavar='<int>', type=int, help='''kmer of filter''', default=31)
pars.add_argument('-s', metavar='<int>', type=int, help='''length of the sliding window on the reads''', default = 4)
pars.add_argument('-tf', metavar='<int>', type=int, help='''thread of filtering''', default = 8)
pars.add_argument('-o', metavar='<str>', type=str, help='''out dir.''', required=False, default = './results/')
pars.add_argument('-py', metavar='<str>', type=str, help='''name of python''', required=False, default= 'python')
pars.add_argument('-lkd', metavar='<str>', type=str, help='''load kmer dict''', required=False, default = 'kmer_dict.txt')
pars.add_argument('-gr', type=STR2BOOL, nargs='?', const=True, help='''get reverse (fast speed) or not (low memory)''', default = True)
...
def Write_Log(log_path, *log_str):
    ...
    with open(log_path, 'a') as out:
        for i in log_str[:-1]: out.write(str(i) + ',')
...
def Seq_To_Int(dna_str, reverse = False ):
    ...
    """
    if reverse:
        dna_str = dna_str.translate(str.maketrans("ACGTU","32100","RYMKSWHBVDN\n"))[::-1]
        return int(dna_str,4)
    else:
        dna_str = dna_str.translate(str.maketrans("ACGTU","01233","RYMKSWHBVDN\n"))
...
def Read_Seq_File(infile):
    ...
    if Judge_Type(infile) == 0:
        return SeqIO.parse(gzip.open(infile, "rt"), "fastq")  # 读取二代测序数据的每一条reads
...
def Make_Kmer_Dict_v6(_kmer_dict, file_list, kmer_size, get_reverse = False, get_pos = False, show_info = True):
    ...
    for i in range(len(file_list)):
        if os.path.isfile(file_list[i]) == False: continue # 如果是个文件夹则跳过
        ...
        for line in infile:
            ...
            refseq = ''.join(filter(str.isalpha, ''.join(temp_str).upper())) #只保留字母
            intseqs, ref_len = Seq_To_Int(refseq) # 序列转整数，获取长度
            if get_reverse: intseqs.append(Seq_To_Int(refseq, True)) # 加入反向互补序列
            for x, y in enumerate(intseqs):
                # 初始化符号位和文件位，反向互补序列的31位符号位为1
                SIGN_BIN = (1 << 30) + (1 << 10) + file_id if x else (1 << 10) + file_id
                for j in range(0, ref_len-kmer_size):
                    ...
                    if kmer_int in _kmer_dict:
                        ...
                        if get_pos: temp_int += int((j+1)/len(refseq)*1000) #记录kmer的千分比位置
                        _kmer_dict[kmer_int] = temp_int
        if show_info: print('Num. of Ref. Seq.:', gene_count, end='\r')
        infile.close()
    return 1
...
def Get_Ref_Info(ref_path, _ref_path_list, _ref_path_dict, _ref_length_dict):
    ...
    for file in _ref_path_list:
        ...
        if Judge_Type(file) == 2:
            ...
            _ref_length_dict[file_name] = int(_ref_length_dict[file_name] /max(ref_seq_count,1))
    return 1
...
def Filter_Reads_v1(_kmer_dict, kmer_size, step_size, read_seq, mask, get_reverse = False, single_ref_file = False):
    ...
    """
    if not read_seq: return ""
    result_int = 0
    read_int, ref_len = Seq_To_Int(read_seq) # 序列转整数，获取长度
    if get_reverse: read_int.append(Seq_To_Int(read_seq, True))
    for s in read_int:
        for j in range(0, ref_len - kmer_size - step_size, step_size):
            ...
            if kmer in _kmer_dict: 
                ...
                if single_ref_file: break
    return bin(result_int)[:1:-1] #返回二进制形式

def Reads_Filter_v5(_kmer_dict, _ref_reads_count_dict, file_list, kmer_size, setp_size, file_1, file_2, t_id, t_count, get_reverse = True):
    """
    过滤Paired Reads
    :param _kmer_dict: 用来保存kmer的字典变量
    ...
    for x, y in enumerate(file_list):
        ...
        file_dict[x] = open(os.path.join(args.o, 'filtered', file_name + "." + str(t_id + 1) + Filted_File_Ext), "w")
    # 读取输入文件
    for _ in infile_1:
        reads_count += 1
        # 构建reads列表
        ...
        if reads_count % t_count == t_id: # 分开线程
            # 获取文件位置的二进制字符串
                ...
                if y == '1':
                    ...
                    if paired_reads_file: 
                        ...
                        file_dict[x].writelines(['>',
                                                ...
                                                Bytes_Str(temp_rec1[1],bytes_type)])
        # 每m reads输出估算速度
        if reads_count << int(paired_reads_file) & 0b11111111111111111111 == 0:
            t2 = time.time()
            t1, t2 = t2, t2 - t1
            print('handled\t',reads_count << int(paired_reads_file) >> 20, 'm reads, ',round(t2,2),'s/m reads'," "*4,sep="", end='\r')
    # 关闭输出和输入文件
...
def Write_Dict(_dict, file_name, is_int = True):
    """
    写入16进制kmer字典
    :param _dict: kmer字典
    :param file_name: 文件名，
...
# 对于macOS, 多进程默认启动方式是spawn, macbook m1上会出错
if sys.platform in ('darwin'):
    multiprocessing.set_start_method('fork')
manager = multiprocessing.Manager()
ref_reads_count_dict = manager.dict() # reads计数的字典，多线程共享写入

if __name__ == '__main__':
    args = pars.parse_args()
    if not args.r:
        print('Must have reference!')
        sys.exit(0)
    # 初始化输出文件夹
    if not os.path.isdir(args.o): os.mkdir(args.o)
    if not os.path.isdir(os.path.join(args.o,'filtered')): os.mkdir(os.path.join(args.o,'filtered'))
    ...
    if args.lkd: 
        ...
        if os.path.exists(os.path.join(args.lkd)):
            ...
            print("The dictionary file of length", len(kmer_dict), "has been successfully loaded.")
    ...
    if not args.lkd:
        ...
        # 保存kmer字典
        Write_Dict(kmer_dict, os.path.join(args.o, "kmer_dict.txt"))
    t1 = time.time()
    print('\nSetp1 cost:', t1 - t0, " "*32) # 建立kmer字典的时间
    # 判断输入测序文件的类型
    if not args.q1: 
        print("At least have one input file")
        sys.exit(0)
    ...
    for i in range(len(args.q1)):
        if args.tf == 1:
            Reads_Filter_v5(kmer_dict, ref_reads_count_dict, ref_path_list, 
                            args.kf, args.s, args.q1[i], args.q2[i], 
                            0, 1, not args.gr)
        else:
            for t in (range(args.tf)):
                p = Process(target=Reads_Filter_v5, 
                            args=(kmer_dict, ref_reads_count_dict, ref_path_list, 
                                  args.kf, args.s, args.q1[i], args.q2[i], 
                                  t, args.tf, not args.gr))
    ...
    for _ in ref_path_list:
        ...
        file_name = file_name.split('.')[0]
        with open(os.path.join(args.o, 'filtered', file_name + Filted_File_Ext),'wb') as wfd:
            for f in [os.path.join(args.o, 'filtered', file_name + "."+str(i) + Filted_File_Ext) for i in range(1, args.tf + 1)]:
                with open(f,'rb') as fd:
                    shutil.copyfileobj(fd, wfd)
                os.remove(f)
    Write_Dict(ref_reads_count_dict, os.path.join(args.o, "ref_reads_count_dict.txt"), False)
    t2 = time.time()
    print('Setp2 cost:', t2 - t1, " "*32) # 过滤所用的时间
    t3 = time.time()
    
    print('Setp3 cost:', t3 - t2, " "*32) # 保存文件所用的时间
    print('Total cost:', t3 - t0, " "*32) # 全部过程所用的时间
    print('Memory:', Get_Dict_Memory_Size(kmer_dict)>>20, 'M')